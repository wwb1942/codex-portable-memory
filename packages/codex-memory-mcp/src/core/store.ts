...
function toTableRow(entry: MemoryEntry): Record<string, unknown> {
  return {
    id: entry.id,
    text: entry.text,
    vector: entry.vector,
...
export class MemoryStore {
  ...
  private async doInitialize(): Promise<void> {
    ...
    try {
      ...
      try {
        table = await db.createTable(TABLE_NAME, [toTableRow(schemaEntry)]);
  ...
  async store(
    ...
    try {
      await this.table!.add([toTableRow(fullEntry)]);
    } catch (err: any) {
      const code = err.code || "";
      const message = err.message || String(err);
      throw new Error(
  ...
  async importEntry(entry: MemoryEntry): Promise<MemoryEntry> {
    ...
    await this.table!.add([toTableRow(full)]);
    return full;
  }
  ...
  async update(
    ...
    return this.runSerializedUpdate(async () => {
      ...
      await this.table!.delete(`id = '${resolvedId}'`);
      try {
        await this.table!.add([toTableRow(updated)]);
      } catch (addError) {
        ...
        try {
          await this.table!.add([toTableRow(rollbackCandidate)]);
        } catch (rollbackError) {
          throw new Error(
