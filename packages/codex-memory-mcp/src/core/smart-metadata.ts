...
export function parseSmartMetadata(
  ...
  const kind = deriveMemoryKind({
    category: entry.category,
    importance: entry.importance,
    explicitKind:
      parsed.kind === "episodic" || parsed.kind === "semantic"
        ? parsed.kind
        : undefined,
  });
  const normalized: SmartMemoryMetadata = {
    ...
    project_id: normalizeOptionalString(parsed.project_id),
...
export function buildSmartMetadata(
  ...
  const nextKind = deriveMemoryKind({
    category: entry.category,
    importance: entry.importance,
    explicitKind:
      patch.kind === "episodic" || patch.kind === "semantic"
        ? patch.kind
        : base.kind,
  });
  ...
  return {
    ...
    project_id:
      patch.project_id === undefined
        ? base.project_id
        : normalizeOptionalString(patch.project_id),
    session_id:
